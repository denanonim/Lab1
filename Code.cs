    ...
    public class Code
    {
        ...
        public string StrCode(string str)
        {
            ...
            foreach (char ch in str)
            {
                ...
                {
                    ou +=CharCode(ch)+" ";
                    output += Convert.ToString(Int16.Parse(CharCode(ch)), 2).PadLeft(8, '0');
                }
        ...
        {
            if (str == "") return "Пустая строка";
            if (!(str.Length%8 == 0)) return "Введено неправильно";
            string output = "";
            for (int i = 7; i < str.Length; i += 8) output+= CharDecode(str.Substring(i - 7, 8));
            return output;
        }
    }
}
